...
- **Mois** : la page `/calendar` affiche les entrées **gardées** du mois sélectionné sous forme de cards triées par `rating` décroissant, avec navigation mois précédent/suivant, pastille « Mois en cours » et suppression définitive protégée par confirmation. Voir l'ADR `Page Mois : liste mensuelle des apprentissages gardés`.
